...
impl StatisticsManager {
    ...
    pub fn get_statistics(&self, project_data: &ProjectData) -> Result<TaskStatistics> {
            ...
            if let Ok(cache) = self.cached_stats.read() {
                if let Some(cached) = cache.get(&cache_key) {
                    ...
                    if age.num_seconds() < self.config.cache_ttl_seconds as i64 {
                        // Update access statistics in a separate scope
                        {
        ...
        if let Ok(mut cache) = self.cached_stats.write() {
            cache.insert(cache_key, CachedStatistic {
                data: stats.clone(),
                created_at: Utc::now(),
                access_count: 1,
    ...
    fn calculate_section_stats(&self, section: &HashMap<String, Task>) -> Result<SectionStats> {
        ...
        Ok(SectionStats {
            ...
            blocked,
            completion_rate,
            avg_completion_time_days: None, // TODO: implement completion time tracking
            most_active_files: vec![], // TODO: implement file activity tracking
        })
    }
        ...
        if let Ok(history) = self.update_history.read() {
            for update in history.iter().take(10) {
                let activity_type = match update.change_type {
                    ...
                    ChangeType::StatusUpdated if update.new_status == TaskStatus::Done => ActivityType::TaskCompleted,
                    ChangeType::StatusUpdated => ActivityType::StatusChanged,
                    _ => ActivityType::SectionUpdated,
                };
                
                activities.push(TaskActivity {
                    description: format!("Task {}:{} {}", update.section, update.task_id, 
    ...
    fn calculate_trends(&self) -> Result<StatsTrends> {
        ...
        Ok(StatsTrends {
            ...
            productivity_score: 75.0, // Mock score
            busiest_sections: vec![], // TODO: implement section activity analysis
        })
    }

    /// Generate cache key for project data
    fn generate_cache_key(&self, project_data: &ProjectData) -> String {
        // Simple hash based on task count and last updated
        format!("stats_{}_{}", 
    ...
    fn cleanup_cache(&self, cache: &mut HashMap<String, CachedStatistic>) {
        ...
        let entries: Vec<_> = cache.iter().map(|(k, v)| (k.clone(), v.last_accessed)).collect();
    ...
    pub fn get_performance_metrics(&self) -> Result<serde_json::Value> {
        let stats = self.performance_stats.read().map_err(|_| anyhow::anyhow!("Failed to read performance stats"))?;
        let cache = self.cached_stats.read().map_err(|_| anyhow::anyhow!("Failed to read cache"))?;
...
mod tests {
    ...
    fn test_statistics_manager_creation() {
        ...
        assert!(metrics["cache_statistics"]["total_cache_entries"].as_u64().unwrap() == 0);
    }

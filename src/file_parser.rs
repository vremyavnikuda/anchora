use regex::Regex;
use crate::task_manager::{TaskStatus, ProjectData};
...
impl TaskParser {
    pub fn new() -> anyhow::Result<Self> {
        Ok(Self {
            full_definition_regex: Regex::new(r"//\s*([\p{L}_][\p{L}\p{N}_]*):([\p{L}_][\p{L}\p{N}_]*):\s+(.+)")?,
            with_status_regex: Regex::new(r"//\s*([\p{L}_][\p{L}\p{N}_]*):([\p{L}_][\p{L}\p{N}_]*):([\p{L}_][\p{L}\p{N}_]*):\s+(.+)")?,
            simple_reference_regex: Regex::new(r"//\s*([\p{L}_][\p{L}\p{N}_]*):([\p{L}_][\p{L}\p{N}_]*)\s*$")?,
            with_note_regex: Regex::new(r"//\s*([\p{L}_][\p{L}\p{N}_]*):([\p{L}_][\p{L}\p{N}_]*):([\p{L}\p{N}_]+)\s*$")?,
            status_update_regex: Regex::new(r"(?i)//\s*([\p{L}_][\p{L}\p{N}_]*):([\p{L}_][\p{L}\p{N}_]*):(todo|in_progress|inprogress|progress|done|completed|complete|blocked|block)\s*$")?,
        })
    }

    pub fn parse_line(&self, line: &str) -> Option<ParsedTaskLabel> {
        let line = line.trim();
        if line.starts_with("/*") || line.ends_with("*/") || line.starts_with("* ") || line.starts_with("*") {
            return None;
        }
    ...
    pub fn scan_file(&self, _file_path: &str, content: &str) -> anyhow::Result<Vec<(u32, ParsedTaskLabel)>> {
    ...
    pub fn update_project_from_labels(
        ...
        for (line_number, label) in labels {
            if let Some(description) = &label.description {
                if project_data.get_task(&label.section, &label.task_id).is_none() {
